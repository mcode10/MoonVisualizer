...
public class SlopeMapButton : MonoBehaviour
{
        ...
        for (int y = 0; y < alphamaps.GetLength(1); y++)
        {
            for (int x = 0; x < alphamaps.GetLength(0); x++)
            {
                ...
                for (int layer = 0; layer < alphamaps.GetLength(2); layer++)
                {
                    ...
                    alphamaps[x, y, layer] = (layer == 1) ?  0f: 
                    alphamaps[x, y, layer] = (layer == 2) ?  Mathf.Clamp01(steepness*steepness/(terrainData.heightmapResolution/10.0f)) : 
